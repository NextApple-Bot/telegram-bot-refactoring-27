...
def _parse_extra_items(form) -> list[dict[str, Any]]:
    """Доп. товары из ассортимента: extra_item_id[] + extra_item_price[]."""
...
async def sale_search_items(q: str = "", exclude_id: int = 0):
    """Поиск товаров для добавления в ту же продажу (SN / текст)."""
    query = (q or "").strip()
    if len(query) < 2:
        return {"items": []}
    ...
    async with async_session() as session:
        stmt = (
            ...
            .where(
                or_(
                    Item.text.ilike(f"%{query}%"),
                    Item.serial.ilike(f"%{query}%"),
                )
            )
...
async def _process_sale_logic(
    ...
    """
    Расчёт оплат:
      товары = цена устройства + доп.позиции + аксессуары − скидка − trade-in − ГНЦ
    ...
    for ex in extra_items:
        ...
        resolved_extras.append({
            "item_id": ex_item.id,
            "text": ex_item.text or "",
            "serial": ex_item.serial,
            "category_id": ex_item.category_id,
            "price": price,
        })

    goods_total = float(sale_price) + accessories_total + extra_total - discount_val - trade_val - gnc_val
    if goods_total < 0:
        goods_total = 0.0
    ...
    for idx, ex in enumerate(resolved_extras, start=1):
        await finalize_item_sale(
            ...
            item_id=ex["item_id"],
            item_text=ex["text"],
            item_serial=ex.get("serial"),
            category_id=ex.get("category_id"),
            message_id=int(sale_message_id) + idx,
            payments=None,
            reason="sale_from_admin",
            is_accessory=False,
            delete_item=True,
