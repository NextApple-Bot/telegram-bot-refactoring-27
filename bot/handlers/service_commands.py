...
async def export_full_report_csv() -> str:
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
        ...
        for row in rows:
            writer.writerow([
                row.client_name, row.phone, row.purchase_id,
                row.created_at.strftime("%d.%m.%Y"), row.total_amount, row.items_text
            ])
        return f.name
...
async def undo_last_deletion() -> str:
    """Простая реализация (можно улучшить через DeletedItem)."""
