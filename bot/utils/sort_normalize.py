...
from bot.utils.samsung_normalize import normalize_samsung_galaxy_line
...
def normalize_item_text(text: str) -> str:
    ...
    s = re.sub(r"\(?\s*2\s*SIM\s*\)?", "(Dual SIM)", s, flags=re.IGNORECASE)
    s = re.sub(r"\bDual\s*SIM\b", "Dual SIM", s, flags=re.IGNORECASE)
    ...
    s = re.sub(r"\bApple Watch\s+Series\s+(\d+)\b", r"Apple Watch S\1", s, flags=re.IGNORECASE)
    s = re.sub(r"\bApple Watch\s+S\s*(\d+)\b", r"Apple Watch S\1", s, flags=re.IGNORECASE)
    s = re.sub(r"\bApple Watch\s+SE\s*(\d+)\b", r"Apple Watch SE \1", s, flags=re.IGNORECASE)
    s = re.sub(r"\bApple Watch\s+Ultra\s*(\d+)\b", r"Apple Watch Ultra \1", s, flags=re.IGNORECASE)
    s = re.sub(r"\bApple Watch\s+Ultra\b(?!\s*\d)", "Apple Watch Ultra", s, flags=re.IGNORECASE)
    ...
    s = re.sub(r"\biPhone\s+(\d+)\s*Pro\s*Max\b", r"iPhone \1 Pro Max", s, flags=re.IGNORECASE)
    s = re.sub(r"\biPhone\s+(\d+)\s*Pro\b(?!\s*Max)", r"iPhone \1 Pro", s, flags=re.IGNORECASE)
    s = re.sub(r"\biPhone\s+(\d+)\s*Plus\b", r"iPhone \1 Plus", s, flags=re.IGNORECASE)
    s = re.sub(r"\biPhone\s+(\d+)\s*Air\b", r"iPhone \1 Air", s, flags=re.IGNORECASE)
    ...
    _color_map = [
        ("natural titanium", "Natural Titanium"), ("white titanium", "White Titanium"),
        ("black titanium", "Black Titanium"), ("desert titanium", "Desert Titanium"),
        ("blue titanium", "Blue Titanium"), ("gray titanium", "Gray Titanium"),
        ("grey titanium", "Gray Titanium"), ("cosmic orange", "Cosmic Orange"),
    ...
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+S(\d+)\s*Ultra\b", r"Samsung Galaxy S\2 Ultra", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+S(\d+)\s*FE\b", r"Samsung Galaxy S\2 FE", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+S(\d+)\s*\+\b", r"Samsung Galaxy S\2+", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+S(\d+)\b(?!\s*(?:Ultra|FE|\+))", r"Samsung Galaxy S\2", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+Z\s*Fold\s*(\d+)\b", r"Samsung Galaxy Z Fold \2", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+Fold\s*(\d+)\b", r"Samsung Galaxy Z Fold \2", s, flags=re.IGNORECASE)
    s = re.sub(r"\b(Samsung\s+)?Galaxy\s+Z\s*Flip\s*(\d+)\b", r"Samsung Galaxy Z Flip \2", s, flags=re.IGNORECASE)
    ...
    if re.search(r"\bSamsung\s+Galaxy\b", s, re.IGNORECASE):
        s = normalize_samsung_galaxy_line(s)
